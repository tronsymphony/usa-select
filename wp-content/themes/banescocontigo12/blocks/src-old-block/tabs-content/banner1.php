        ...
        <div class="container">
            <div class="text-image-slider__inner">
                <div class="text-image-slider__image"><img decoding="async"
                        src="<?= $attributes['items1'][0]['banner_image']['url']; ?>" alt="image"></div>
                <div class="text-image-slider__text">
                    ...
                    <?php if(!empty($attributes['items1'][0]['initial_opened'])): ?>
                    <div class="text-image-slider__modal text-image-slider__modal-popup-feature onelink">+ Ver todos los
                        detalles</div>
                    <?php endif; ?>
                </div>
            </div>
...
<div class="text-image-slider__modal-content modalone">
    <?php if(!empty($attributes['items1'][0]['modal_title'])): ?>
        <h2 class="text-image-slider__modal-content-title"><?php echo $attributes['items1'][0]['modal_title']; ?></h2>
    <?php endif; ?>
    <?php if(!empty($attributes['items1'][0]['modal_text'])): ?>
        <div>
            <?php echo $attributes['items1'][0]['modal_text']; ?>
        </div>
    <?php endif; ?>
    <span class="text-image-slider__modal-close closeone">
...
<script>
const onelink = document.getElementsByClassName("onelink");

if (onelink.length > 0) {
    document.querySelectorAll('.onelink').forEach(function(item) {
    ...
    document.querySelector('.closeone')?.addEventListener('click', function(e) {
        e.preventDefault()
