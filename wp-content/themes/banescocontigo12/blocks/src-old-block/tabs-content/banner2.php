        ...
        <div class="container">
            <div class="text-image-slider__inner">
                <div class="text-image-slider__image"><img decoding="async"
                        src="<?= $attributes['items2'][0]['banner_image']['url']; ?>" alt="image"></div>
                <div class="text-image-slider__text">
                    <h2 class="text-image-slider__title"><?= $attributes['items2'][0]['banner_title']; ?></h2>
                    <div class="text-image-slider__content">
                        <?= $attributes['items2'][0]['banner_text']; ?>
                    </div>
                    <?php
                    if(!empty($attributes['items2'][0]['initial_opened'])): ?>
                    <div class="text-image-slider__modal text-image-slider__modal-popup-feature twolink">+ Ver todos los
                        detalles</div>
                    <?php endif; ?>
                </div>
            </div>
...
<div class="text-image-slider__modal-content modaltwo">
<?php if(!empty($attributes['items2'][0]['modal_title'])): ?>
        <h2 class="text-image-slider__modal-content-title"><?php echo $attributes['items2'][0]['modal_title']; ?></h2>
    <?php endif; ?>
    <?php if(!empty($attributes['items2'][0]['modal_text'])): ?>
        <div>
            <?php echo $attributes['items2'][0]['modal_text']; ?>
        </div>
    <?php endif; ?>
    <span class="text-image-slider__modal-close closetwo">
...
<div class="text-image-slider__modal-content-overlay overlay2"></div>

<script>
const twolink = document.getElementsByClassName("twolink");

if (twolink.length > 0) {
    document.querySelectorAll('.twolink').forEach(function(item) {
        item.addEventListener('click', function(e) {
            e.preventDefault()
            document.querySelector('.modaltwo').classList.add('active-modal')
            document.querySelector('.overlay2').classList.add(
                'active-modal')
        })
    })

    document.querySelector('.closetwo')?.addEventListener('click', function(e) {
        e.preventDefault()

        document.querySelector('.modaltwo').classList.remove('active-modal')
        document.querySelector('.overlay2').classList.remove('active-modal')
    })
}
</script>